...
export class ProvidersComponent implements OnInit {
  ...
  languageId:any='1'
  languageList:any;
  constructor(private service:ProviderService,private langService:LanguageService, private router:Router,private uService: UtilitydesignService) 
  {
  ...
  ngOnInit(): void {
   ...
   getProvidersList(){
    ...
    this.service.getIrinnProvideByLanguageId(formData).subscribe((res:any)=>{
     this.uService.isLoading= false;
    
    if (res.status) {
      this.providerList = res.data.content;
      if (res?.data) {
  ...
  onUpdateStatus(item:any){
    ...
    this.service.updateIrinnProvideByStatus(formData, item.id).subscribe((data: any) => {
     this.uService.isLoading= false;
