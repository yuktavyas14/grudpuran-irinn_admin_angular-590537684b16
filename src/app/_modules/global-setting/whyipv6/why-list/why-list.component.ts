...
@Component({
  selector: 'app-why-list',
  templateUrl: './why-list.component.html',
  styleUrls: ['./why-list.component.scss']
})
export class WhyListComponent implements OnInit {
  languageId:any='1'
  languageList:any;
  ipv6List:any;
  totalrecored: any = 0;
  ...
  ngOnInit(): void {
   ...
   getIpList(){
    ...
    this.service.getwhyIPv6List(formData).subscribe((res:any)=>{
  ...
  onView(item:any){
    this.router.navigate(['/global-setting/why-ipv6/edit',item.id,item.languageId])
  }
  onUpdateStatus(item:any){
    ...
    this.service.updateWhyIPv6StatusById(formData).subscribe((data: any) => {
     this.uService.isLoading= false;
