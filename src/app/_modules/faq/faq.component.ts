...
constructor(private service:FaqService,private langService: LanguageService, 
  private router:Router,private uService: UtilitydesignService) 
{
  this.getFaqListlanguageid();
 }
ngOnInit(): void {
 ...
 getFaqListlanguageid(){
   ...
   let page:any= this.page -1
    ...
    formData.append('languageId', this.languageId,);
  this.service.getFaqListBylanguageid(formData).subscribe((res:any)=>{
...
onView(faq:any){
  this.router.navigate(['/faq/edit',faq.id,faq.languageId ||'1'])
}
onUpdateStatus(item:any){
  ...
  this.service.updateFaqStatus(formData, item.id).subscribe((data: any) => {
   this.uService.isLoading= false;
